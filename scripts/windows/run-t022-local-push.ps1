...
$logPath = Join-Path $LogDir "t022_local_push.log"
if ((Test-Path $logPath) -and ((Get-Item $logPath).Length -gt ($MaxLogSizeMb * 1MB))) {
  $archiveName = "t022_local_push.{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss")
  Move-Item -Path $logPath -Destination (Join-Path $LogDir $archiveName) -Force
}

$mutexName = "Global\OnlyTrade.T022.LocalPush"
...
try {
  ...
  if (-not $hasLock) {
    Write-LogLine -Path $logPath -Message "skip: another t022 local push is already running"
    exit 0
  }
  ...
  $bashCommand = "cd '$repoBash' && bash scripts/content_factory/local_render_and_push_t022.sh"
