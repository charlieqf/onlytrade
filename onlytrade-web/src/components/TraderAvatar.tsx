...
export function TraderAvatar({
  ...
  const openPreview = () => {
    if (!enableHdPreview || profile.style !== 'photo' || !profile.hdPreviewUrl) return
    const rect = avatarRef.current?.getBoundingClientRect()
    if (!rect) return

    const previewWidth = Math.min(420, Math.max(280, Math.floor(window.innerWidth * 0.32)))
    ...
    const top = Math.max(12, Math.min(window.innerHeight - previewHeight - 12, rect.top - 24))
  ...
  if (profile.style === 'photo' && profile.photoUrl) {
    return (
      <>
        ...
        {enableHdPreview && showPreview && previewPos && profile.hdPreviewUrl && (
          <div
