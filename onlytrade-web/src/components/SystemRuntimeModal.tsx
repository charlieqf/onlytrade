  ...
  const applyCadence = () => {
    ...
    if (!Number.isFinite(bars) || bars <= 0) {
      setMessage(
        language === 'zh'
          ? '请输入有效的决策间隔。'
          : 'Please enter a valid cadence value.'
      )
      return
    }
    void run(
      ...
      language === 'zh'
        ? `决策节奏已更新为每 ${bars} 根K线`
        : `Cadence updated to every ${bars} bars`
    )
  }
      ...
      <div className="relative w-[min(980px,96vw)] max-h-[90vh] overflow-y-auto nofx-glass p-4 border border-white/15 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-2 mb-3">
          <input
            ...
            placeholder={
              language === 'zh' ? '每次决策间隔 K 线数' : 'bars per decision'
            }
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          ...
          <button
            onClick={() =>
              void run(
                async () => onRuntimeControl('resume'),
                language === 'zh' ? '已恢复' : 'Resumed'
              )
            }
          ...
          <button
            onClick={() =>
              void run(
                async () => onRuntimeControl('step'),
                language === 'zh' ? '已单步执行' : 'Stepped once'
              )
            }
            disabled={pending || isKillActive}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-3">
          <button
            onClick={() =>
              void run(
                async () => onFactoryReset(false),
                language === 'zh' ? '已重置到首根K线' : 'Reset to first bar'
              )
            }
            disabled={pending}
          ...
          <button
            onClick={() =>
              void run(
                async () => onFactoryReset(true),
                language === 'zh' ? '已重置到warmup' : 'Reset to warmup'
              )
            }
            disabled={pending}
            ...
            onClick={() =>
              void run(
                async () =>
                  onKillSwitch(
                    'activate',
                    'manual_emergency_stop_from_system_popup'
                  ),
                language === 'zh'
                  ? 'Kill Switch 已激活'
            ...
            onClick={() =>
              void run(
                async () =>
                  onKillSwitch(
                    'deactivate',
                    'manual_reactivate_from_system_popup'
                  ),
                language === 'zh'
                  ? 'Kill Switch 已解除'
        ...
        <div className="text-[11px] text-nofx-text-muted space-y-1">
          <div>
            {language === 'zh' ? '运行状态' : 'Runtime'}:{' '}
            <span className="text-nofx-text-main ml-1">
              {runtimeStatus?.running ? 'Running' : 'Paused'}
            </span>
            <span className="mx-2">|</span>
            {language === 'zh' ? '循环间隔' : 'Cycle'}:{' '}
            <span className="text-nofx-text-main ml-1">
              {runtimeStatus?.cycle_ms ?? '--'} ms
            </span>
          </div>
          <div>
            {language === 'zh' ? '决策节奏' : 'Cadence'}:{' '}
            <span className="text-nofx-text-main ml-1">
              {runtimeStatus?.decision_every_bars ?? '--'} bars
            </span>
            <span className="mx-2">|</span>
            LLM:{' '}
            <span className="text-nofx-text-main ml-1">
          ...
          <div>
            Kill Switch:{' '}
            <span
              className={`ml-1 ${isKillActive ? 'text-red-300' : 'text-nofx-green'}`}
            >
          ...
          <div>
            ...
            <span className="text-nofx-text-main ml-1">
              {replayRuntimeStatus?.trading_day ?? '--'}
            </span>
            <span className="mx-2">|</span>
            {language === 'zh' ? '第' : 'Day'}{' '}
            <span className="text-nofx-text-main ml-1">
              {replayRuntimeStatus?.day_index ?? '--'}
            </span>
            /{replayRuntimeStatus?.day_count ?? '--'}
          </div>
          <div>
            {language === 'zh' ? '回放模式' : 'Replay mode'}:{' '}
            <span className="text-nofx-text-main ml-1">
              {replayRuntimeStatus?.loop
                ? language === 'zh'
                  ? '循环'
                  : 'Loop'
                : language === 'zh'
            ...
            <span
              className={`ml-1 ${replayRuntimeStatus?.completed ? 'text-nofx-green' : 'text-nofx-text-main'}`}
            >
              {replayRuntimeStatus?.completed
                ? language === 'zh'
                  ? '已完成'
                  : 'Completed'
                : language === 'zh'
