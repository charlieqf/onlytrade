...
export const api = {
  ...
  async bootstrapChatSession(): Promise<ChatSessionBootstrapResult> {
    const result = await httpClient.post<ChatSessionBootstrapResult>(
  ...
  async getTopicStreamLive(options?: {
    room_id?: string
  }): Promise<{
    room_id: string
    live: Record<string, unknown>
    ...
    if (options?.room_id) params.set('room_id', String(options.room_id).trim())
    const res = await fetch(
      `${API_BASE}/topic-stream/live?${params.toString()}`,
      {
        method: 'GET',
        headers: getAuthHeaders(),
        cache: 'no-store',
    ...
    if (payload && typeof payload === 'object' && 'success' in payload) {
      ...
      if (!wrapped.success || !wrapped.data) {
        throw new Error(wrapped.message || '获取专题流数据失败')
      }
      return {
        room_id: String(wrapped.data.room_id || options?.room_id || ''),
        live: wrapped.data.live || {},
        status: wrapped.data.status || {},
      }
    }
    ...
    return {
      room_id: String(direct.room_id || options?.room_id || ''),
      live: direct.live || {},
      status: direct.status || {},
    }
  },

  async getContentFactoryLive(options?: {
    room_id?: string
  }): Promise<{
    room_id: string
    live: Record<string, unknown>
    ...
    if (options?.room_id) params.set('room_id', String(options.room_id).trim())
    const res = await fetch(
      `${API_BASE}/content-factory/live?${params.toString()}`,
      {
        method: 'GET',
        headers: getAuthHeaders(),
        cache: 'no-store',
    ...
    if (payload && typeof payload === 'object' && 'success' in payload) {
      ...
      if (!wrapped.success || !wrapped.data) {
        throw new Error(wrapped.message || '获取内容工厂数据失败')
      }
      return {
        room_id: String(wrapped.data.room_id || options?.room_id || ''),
        live: wrapped.data.live || {},
  ...
  async getRoomStreamPacket(
    roomId: string,
    decisionLimit: number = 5
  ): Promise<RoomStreamPacket> {
    const safeRoomId = String(roomId || '').trim()
    if (!safeRoomId) throw new Error('room_id_required')
  ...
  async getDecisionAuditLatest(
    traderId: string,
    limit: number = 50
  ): Promise<DecisionAuditListPayload> {
  ...
  async getAgentRuntimeStatus(): Promise<AgentRuntimeStatus> {
    const result = await httpClient.get<AgentRuntimeStatus>(
      `${API_BASE}/agent/runtime/status`
    )
  ...
  async getReplayRuntimeStatus(): Promise<ReplayRuntimeStatus> {
    const result = await httpClient.get<ReplayRuntimeStatus>(
      `${API_BASE}/replay/runtime/status`
    )
    if (!result.success || !result.data)
      throw new Error('获取回放运行时状态失败')
    return result.data
  },
