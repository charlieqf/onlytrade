...
MARKET_OPTIONAL_COLUMNS = {
    "source_topic": "TEXT",
    "source_source": "TEXT",
    "source_hot_score": "TEXT",
    "news_summary": "TEXT",
    "news_key_points": "TEXT",
}


def _ensure_market_optional_columns(cursor):
    cursor.execute("PRAGMA table_info(MARKETS)")
    rows = cursor.fetchall()
    existing = {str(row[1] or "").strip().lower() for row in rows if len(row) >= 2}
    for column, column_type in MARKET_OPTIONAL_COLUMNS.items():
        if column.lower() in existing:
            continue
        cursor.execute(f"ALTER TABLE MARKETS ADD COLUMN {column} {column_type}")
...
def init_db():
    ...
    with db_lock:
        ...
        """)
        _ensure_market_optional_columns(cursor)
        ...
        cursor.execute(
            ...
            ("user_ai", "AI_Agent_Zero", 1000000.0),
        )  # AI has a larger starting bankroll
        cursor.execute(
            "INSERT OR IGNORE INTO USERS (id, username, balance) VALUES (?, ?, ?)",
...
def create_market(market_json):
    ...
    with db_lock:
        ...
        raw_points = market_json.get("news_key_points")
        key_points_text = (
            json.dumps(raw_points, ensure_ascii=False)
            if isinstance(raw_points, (list, dict))
            else str(raw_points or "").strip()
        )
        ...
        """,
            (
                ...
                str(market_json.get("source_topic") or "").strip() or None,
                str(market_json.get("source_source") or "").strip() or None,
                str(market_json.get("source_hot_score") or "").strip() or None,
                str(market_json.get("news_summary") or "").strip() or None,
...
def place_bet(user_id, market_id, bet_type, amount):
    ...
    with db_lock:
        ...
        cursor.execute(
            "SELECT status, current_prob FROM MARKETS WHERE id = ?", (market_id,)
        )
...
def _recalculate_odds(conn, cursor, market_id, old_prob):
    ...
    cursor.execute(
        "SELECT SUM(amount) FROM BETS WHERE market_id = ? AND bet_type = 'YES'",
        (market_id,),
    )
    yes_volume = cursor.fetchone()[0] or 0.0

    cursor.execute(
        "SELECT SUM(amount) FROM BETS WHERE market_id = ? AND bet_type = 'NO'",
        (market_id,),
    )
    ...
    if total_volume > 0:
        new_yes_prob = ((initial_prob * BASE_LIQUIDITY) + yes_volume) / (
            BASE_LIQUIDITY + total_volume
        )
    else:
    ...
    if abs(new_yes_prob - old_prob) > 0.001:
        cursor.execute(
            "UPDATE MARKETS SET current_prob = ? WHERE id = ?",
            (new_yes_prob, market_id),
        )
        conn.commit()
        logging.info(
            f"Odds Updated for {market_id}: {old_prob:.4f} -> {new_yes_prob:.4f}"
        )
...
if __name__ == "__main__":
    ...
    for m in markets:
        print(
            f"Market: {m['title'][:20]}... | Initial Prob: {m['initial_prob']:.2f} | Current Prob: {m['current_prob']:.4f}"
        )
