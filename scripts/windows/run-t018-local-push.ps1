...
$logPath = Join-Path $LogDir "t018_local_push.log"
if ((Test-Path $logPath) -and ((Get-Item $logPath).Length -gt ($MaxLogSizeMb * 1MB))) {
  $archiveName = "t018_local_push.{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss")
  Move-Item -Path $logPath -Destination (Join-Path $LogDir $archiveName) -Force
}

$mutexName = "Global\OnlyTrade.T018.LocalPush"
...
try {
  ...
  if (-not $hasLock) {
    Write-LogLine -Path $logPath -Message "skip: another t018 local push is already running"
    exit 0
  }
  ...
  $bashCommand = "cd '$repoBash' && bash scripts/topic_stream/local_collect_and_push_t018.sh"
