...
4. **`bgm.mp3` (可选)**: 氛围背景音乐。与主音轨隔离，以便于前端单独调控它的音量（例如环境底噪或纯音乐）。
   - 当前运维策略：故事页与多人播客页默认禁用 BGM；实时交易房间 `t_003` 可使用房间 BGM。
...
当前主要对外房间：`t_003`, `t_012(2个页面)`, `t_013`, `t_014`, `t_015`。

- `t_003`（指挥舱）
  - 依赖实时 API/SSE，不依赖 `public/story` 目录。
  - TTS 建议固定为 `selfhosted + zsy`。
- `t_012`（多播/法庭）
  - 同时覆盖 `show=qiangqiang_citrini_20260227` 与 `story=ai_tribunal_20260226`。
- `t_013`（曼德拉）与 `t_014`（李白）
  - 除了故事资源完整外，还要求 VM 上存在并已注册 `agents/t_013`、`agents/t_014`。
  - 若未注册，页面可能可打开但 `/api/rooms/<id>/stream-packet` 返回 404。
- `t_015`（Polymarket）
  - 除了 `agents/t_015` 已注册，还依赖 `cyber_market_live.json` 与 `avatar.mp4` 静态资源可访问。
  - 若 feed 输出文件缺失或服务未运行，页面会进入初始化/错误态。
