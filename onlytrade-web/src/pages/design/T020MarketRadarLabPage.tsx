...
const PROGRAM_THEMES: Record<string, ThemeSpec> = {
  'market-radar-lab': {
    label: '市场快评实验室',
    eyebrow: 'Market Radar Lab',
    accent: '#7de2ff',
    badge: 'bg-[#0f1d28]/80 text-[#d6f7ff] border-[#7de2ff]/55',
    overlay:
      'bg-[radial-gradient(circle_at_16%_14%,rgba(125,226,255,0.22),transparent_24%),radial-gradient(circle_at_82%_20%,rgba(255,166,77,0.16),transparent_22%),linear-gradient(180deg,rgba(7,10,16,0.26)_0%,rgba(7,10,16,0.12)_30%,rgba(7,10,16,0.84)_100%)]',
    panel:
      'bg-[linear-gradient(180deg,rgba(12,22,34,0.78)_0%,rgba(7,10,16,0.92)_100%)]',
  },
}
...
function getProgramSlugFromUrl(): string {
  if (typeof window === 'undefined') return 'market-radar-lab'
  const params = new URLSearchParams(window.location.search)
  return cleanText(params.get('program') || 'market-radar-lab', 64).toLowerCase() || 'market-radar-lab'
}

export default function T020MarketRadarLabPage(_: FormalStreamDesignPageProps) {
  useFullscreenLock()
  ...
  const theme = PROGRAM_THEMES[programSlug] || PROGRAM_THEMES['market-radar-lab']
  ...
  return (
    <div className="relative h-[100dvh] w-full overflow-hidden bg-[#090b11] text-white">
      {backgroundImageUrl ? (
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,#102235_0%,#0b1018_34%,#05070d_100%)]" />
      )}
