...
def calculate_ai_pnl(cursor):
    ...
    for bet in bets:
        ...
        current_price = current_true_prob if b_type == 'YES' else (1.0 - current_true_prob)
...
def build_state(conn, cursor, market_id):
    ...
    if not logs:
        logs = [{"id": 0, "sender": "System", "type": "info", "text": f"Tracking initialized for {market_data['title']}", "time": int(time.time() * 1000)}]
...
def run_bridge():
    ...
    while True:
        try:
            ...
            if current_market_id is None or (time.time() - last_rotation_time > 15):
                 # Pick a random open market to focus the stream on
                 current_market_id = random.choice(open_markets)['id']
                 last_rotation_time = time.time()
